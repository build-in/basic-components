...
export default {
  ...
  props: {
    // amount【金额】，account【银行账号】，orderNo【单号】，percent【百分比】，integer【整数】，tel【电话号码】,negativeAmount 【带符号金额】
    inputType: {
      type: String,
      default: 'amount',
    },
  ...
  data() {
    return {
      ...
      toNumberForce: ['negativeAmount']
    };
  },
  computed: {
    // 整数长度限制
    integerLimit() {
      if (['amount', 'percent', 'negativeAmount'].includes(this.inputType)) {
        return 13;
      }
    },
    // 最大长度限制
  ...
  watch: {
    ...
    selfValue(value) {
      ...
      if (this.inputType === 'negativeAmount') {
        filterZero(fixVal);
        limitIntegerPlace(fixVal);
        limitDecimalPlace(fixVal);
        limitRange(fixVal);
  ...
  methods: {
    ...
    onKeyDown(e) {
      ...
      if (['negativeAmount'].includes(this.inputType)) {
        const {selectionStart, selectionEnd} = e.target;
        const left = value.substring(0, selectionStart);
        const right = value.substring(selectionEnd);
        const finalValue = `${left}${key}${right}`
        console.log('最终输入值为:', finalValue)
        const integerLimit = this.integerLimit
        const decimalLimit = this.decimalLimit
        // const a = /^[+-]?\d{0,13}((?<=[0-9])\.)?((?<=\.)\d{0,2})?$/
        const regPattern = `^[+-]?\\d{0,${integerLimit}}((?<=[0-9])\\.)?((?<=\\.)\\d{0,${decimalLimit}})?$`
        console.log('校验regPattern:', regPattern)
        //输入后校验不通过，则不准输入
        if (!new RegExp(regPattern).test(finalValue)) {
          console.log('校验不通过:', finalValue)
          preventDefault(e);
          return;
        }
      }
    ...
    changeHandle(val) {
      //强制转number，不能转化则强制为0
      if (this.toNumberForce.includes(this.inputType)) {
        const numValue = Number(val);
        this.selfValue = Object.is(NaN, numValue) ? '0' : String(numValue)
        return;
      }
      if (val) {
    ...
    getIllegalPattern() {
      ...
      if (this.inputType === 'negativeAmount') { //金额但是允许输入负数
        return /[^0-9.-]/g;
      }
      return /[^0-9.-]/g;
    },
